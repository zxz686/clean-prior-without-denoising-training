...
class ModelCAtt(ModelBase):
    def __init__(self, opt):
        super(ModelCAtt, self).__init__(opt)
        # ------------------------------------
        # define network
        # ------------------------------------
        ...
        if self.opt_train['E_decay'] > 0:
            self.netE = define_G(opt).to(self.device).eval()  # 用于指数移动平均的额外模型
    """
    # ----------------------------------------
    ...
    def feed_data(self, data, need_H=True):
        ...
        self.LM = data['LM'].to(self.device)  # 使用新的LM数据
        if need_H:
    ...
    def netG_forward(self):
        # 现在使用LM而不是L
        self.E = self.netG(self.LM)  # 注意：确保你的netG能处理6通道输入
        if self.is_mask:
    ...
    def current_visuals(self, need_H=True):
        ...
        # 假设L是前3个通道，M是后3个通道
        out_dict['L'] = self.LM.detach()[:, :3, :, :][0].float().cpu()  # 提取L部分
        out_dict['M'] = self.LM.detach()[:, 3:, :, :][0].float().cpu()  # 提取M部分
    ...
    def current_results(self, need_H=True):
        out_dict = OrderedDict()
        # 同样地，假设L是前3个通道，M是后3个通道
        out_dict['L'] = self.LM.detach()[:, :3, :, :].float().cpu()  # 提取L部分
        out_dict['M'] = self.LM.detach()[:, 3:, :, :].float().cpu()  # 提取M部分
